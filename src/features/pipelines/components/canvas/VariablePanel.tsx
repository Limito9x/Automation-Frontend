...
const TYPE_OPTIONS = [
  ...
  { value: "4", label: "EntityRef", color: "text-emerald-500 bg-emerald-500/10 border-emerald-500/30" },
...
export function VariablePanel({
  ...
  const getTypeStyle = (t: number | string, c?: number | string) => {
    const tStr = String(t);
    ...
    return {
      label: isMap ? `Map<${opt.label}>` : isArr ? `${opt.label}[]` : opt.label,
      color: opt.color,
    };
  };
          ...
          <div className="flex flex-1 flex-col overflow-hidden p-3 gap-3">
            ...
            <ScrollArea className="flex-1 -mx-1 px-1">
              {variables.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {variables.map((v) => {
                    const style = getTypeStyle(v.type, v.cardinality);
                    return (
                      <div
                        key={v.name}
                        draggable
