...
import {
  createInspectorVersion,
  publishInspectorVersion,
  getGetInspectorByIdQueryKey,
} from "@/gen/endpoints/inspectors/inspectors";
...
export function useInspectorVersions(inspectorId: string, projectId?: string) {
  ...
  const createVersionMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        t("inspectors.versionCreated", {
          defaultValue: "Inspector version uploaded successfully",
        }),
      );
    ...
    onError: (err: any) => {
      const errorMsg =
        ...
        err?.message ||
        "Failed to create inspector version";
      toast.error(
        t("inspectors.versionCreateFailed", { defaultValue: errorMsg }),
      );
    },
  });
  ...
  const publishVersionMutation = useMutation({
    mutationFn: async (versionId: string) => {
      return await publishInspectorVersion(inspectorId, versionId);
    },
    onSuccess: () => {
      toast.success(
        t("inspectors.versionPublished", {
          defaultValue: "Version published successfully",
        }),
      );
      queryClient.invalidateQueries({
        queryKey: getGetInspectorByIdQueryKey(inspectorId),
